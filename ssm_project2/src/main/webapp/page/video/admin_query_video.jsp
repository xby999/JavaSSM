                ...
                <div class="ibox-content">
                    ...
                    <div class="jqGrid_wrapper">
                        <table class="footable table table-stripped table-hover" data-page-size="8" data-filter=#filter>
                            ...
                            <c:forEach items="${admin_list}" var="videoinfo">
                                <tr class="gradeX">
                                    <td>${videoinfo.video_id}</td>
                                    <td>${videoinfo.business_username}</td>
                                    <td>${videoinfo.video_title}</td>
                                    <td>${videoinfo.video_date}</td>
                                    <td>${videoinfo.video_src}</td>
                                    <td class="center">
                                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal"  onclick="changeVideosrc('${videoinfo.video_src}')">查看</button>
                                    </td>
                                    <td>z
                                        <a href="${pageContext.request.contextPath}/VideoController/adimeDeleteVideo.action?video_id=${videoinfo.video_id}"><button type="button">删除</button></a>
                                    </td>
                                </tr>
                            </c:forEach>
